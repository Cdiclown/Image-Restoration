...
def newton(f, df, tolf, tolx, maxit, xTrue, x0=1):
...
def succ_app(f, g, tolf, tolx, maxit, xTrue, x0=1):
...
f = lambda x: np.exp(x)*np.power(x,2)
df = lambda x: (np.exp(x)*2*x )+ (np.exp(x)*np.power(x,2))
...
xtrue = 0
'''
#(x, err, iterations) = bisezione(xtrue, f, -1 , 1,0.0000001)
...
'''
(x, iterations, err, vecErr) = newton(f, df, 10**(-10),10**(-6), 100, xtrue)
...
(x, i, err, vecErr) = succ_app(f, g2, tolx, tolf, maxit, xtrue)

print("Found value with succ. appr. method for f1 with g2: ", x, "\nIterations: ", i)
...
plt.plot(x,f(x),'o')
plt.subplot(1,2,2)
plt.title("Errore con metodo delle appr. succ. (g2)")
plt.plot(vecErr)
plt.grid()
plt.show()

(x, i, err, vecErr) = succ_app(f, g3, tolx, tolf, maxit, xtrue)

print("Found value with succ. appr. method for f1 with g3: ", x, "\nIterations: ", i)
...
plt.plot(x,f(x),'o')
plt.subplot(1,2,2)
plt.title("Errore con metodo delle appr. succ. (g3)")
plt.plot(vecErr)
plt.grid()
plt.show()
